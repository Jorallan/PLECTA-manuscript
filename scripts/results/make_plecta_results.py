...
DEPTH_ROWS = (
    ...
    (SUB, "restricted to crossings the stage decided", "coa_decided", "%.2f"),
    (ROW, "Declined, of crossings the stage found", "abstain_rate", "%.2f"),
    (BLOCK, "In the resulting stack", None, None),
...
def real_fields_macros(payload: dict) -> list[str]:
    ...
    #  Both means are over every field the table reports, so the one sentence
    #  that quotes them can say "averaged over them" of both and be exact.
    #
    #  This mean used to be over a subset -- the fields the upstream segmenter
    #  had not trained on -- with the prose naming the exclusion. Retired
    #  2026-08-27 by author decision, together with that sentence and with
    #  PlectaRealUNetHeldOutCount, which nothing quotes once the sentence is
    #  gone; same rule as PlectaRealHeldOutCount above. At the reported
    #  precision the change is invisible, 0.45878 over two against 0.45819 over
    #  three, so no printed number moves. `unet_held_out` survives per field in
    #  plecta_real_fields.json and the contamination experiment in
    #  plecta_real_masks.json, so restoring either is one line and no re-run.
    all_fields = payload["fields"]
    lines.append(
        macro("PlectaRealUNetFOneMean",
              fmt(sum(f["conditions"]["unet"]["pairwise_f1"]
                      for f in all_fields) / len(all_fields))))
...
def comparator_table(audit: dict, runtime: dict | None = None,
                     basu: dict | None = None,
                     sifne: dict | None = None,
                     real_comparators: dict | None = None) -> str:
    ...
    differ, and the pooled form is the one the prose reports. GraFT's row is
    over the scenes it completed, so its crossing count is smaller, and since
    the scene-count column went nothing on the page says so -- see the note at
    the sort below.
    """
    ...
    #  No scene-count column, by author decision 2026-08-27. It carried 50 for
    #  every method except GraFT, which completed 47, and 3 for the real block;
    #  the two constants are in the prose immediately above the table, as
    #  \PlectaGreedySceneCount and \PlectaRealFieldCountLower. GraFT's 47 is
    #  therefore no longer stated anywhere, and its row is a mean over three
    #  fewer scenes than the rows beside it. The count is still in the audit
    #  record as means[key]["n_scenes"], so restoring the column is one cell
    #  per row and no re-run.
    ...
    ncols = 9 if runtime else 7
    lines = ["% Generated by scripts/results/make_plecta_results.py; do not edit.",
             #  tabular* with a rubber intercolumn skip, as the real-fields
             #  table uses, so the columns spread across the text block rather
             #  than huddling in the middle of it with the booktabs rules
             #  overhanging them. The \\cmidrule below spans columns, not
             #  lengths, so it follows the stretch without adjustment.
    ...
    if runtime:
        lines.extend([
            ...
            r"\cmidrule(l){8-9}",
            r"Method & $F_1$ & Precision & Recall & ARI & "
            r"VI total~$\downarrow$ & fidelity & at 20\,\% & at 60\,\% \\",
        ])
    else:
        lines.append(r"Method & $F_1$ & Precision & Recall & ARI & "
                     r"VI total~$\downarrow$ & Crossing fidelity \\")
    lines.append(r"\midrule")

    #  Panel headings say what the material IS, not how much of it there is:
    #  the counts are in the prose above the table.
...
def heldout_table(held: dict) -> str:
    lines = [
        ...
        r"\toprule",
        r"Group & $n$ & $F_1$ & Precision & Recall & ARI & "
...
def ablation_table(payload: dict, audit: dict | None = None,
                   greedy_baseline: dict | None = None) -> str:
    ...
    It is evaluated on 30 development scenes, and its delta is taken against
    the exact-matcher arm of those same scenes rather than against the fixed
    configuration on the larger development set.
    """
    ...
    lines = [
        "% Generated by scripts/results/make_plecta_results.py; do not edit.",
        #  tabular* with a rubber intercolumn skip, as the real-fields table
        #  uses: four columns, one of them long text and three narrow
        #  numbers, otherwise huddle in the middle of the text block with the
        #  booktabs rules overhanging them by an inch a side.
        r"\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lrrr@{}}",
        r"\toprule",
        r"Variant & $F_1$ & $\Delta F_1$ & ARI \\",
        r"\midrule",
    ]
...
def real_fields_table(payload: dict, comparators: dict) -> str:
    ...
    The two comparators moved out on 2026-08-27, when the comparison of methods
    became the last subsection of the results. Everything that ranks PLECTA
    against another method now lives there, these fields included: they are a
    block of the comparator table, averaged over the three. What is left here
    is one method under two input conditions, which is the only question the
    subsection around this table asks. The comparator record is still read, for
    each field's measured areal coverage.
    """
    ...
    lines = [
        "% Generated by scripts/results/make_plecta_results.py; do not edit.",
        #  tabular* with a rubber intercolumn skip, so six narrow columns are
        #  spread across the text block instead of huddling in the middle of
        #  it. booktabs rules span the full width either way; what changes is
        #  that the rules no longer overhang the content by an inch a side.
        r"\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lrlrrr@{}}",
        r"\toprule",
        r"Field & Coverage & Input axis & $F_1$ & Join $F_1$ & "
        r"Det.\ $F_1$ \\",
        r"\midrule",
    ]
    for position, field in enumerate(payload["fields"]):
        ...
        block: list[tuple[str, dict]] = [
            ("Manual-derived", field["conditions"]["manual"]),
            ("U-Net", field["conditions"]["unet"]),
        ]
        for row_index, (axis, values) in enumerate(block):
...
def main() -> None:
    ...
    (RESULTS / "plecta_comparator_table.tex").write_text(
        comparator_table(audit, runtime, basu, sifne, real_comparators),
        encoding="utf-8"
    )
    #  UNREFERENCED since 2026-08-27: the width-against-density table was
    #  removed from Section 3.1 by author decision. Its point is now one
    #  sentence -- PLECTA is handed a centreline mask and never a width, so
    #  three bundle widths over one geometry move areal coverage and not the
    #  score -- and every number that sentence quotes is still a macro, so
    #  nothing was lost with the float. Kept emitting for the same reason as
    #  plecta_greedy_table.tex above: the table is a correct self-contained
    #  record. Delete the block and the file together if that is decided.
