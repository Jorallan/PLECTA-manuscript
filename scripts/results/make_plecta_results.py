...
def ablation_table(payload: dict, audit: dict | None = None,
    ...
    lines = [
        ...
        #  uses: five columns, one of them long text and four of them narrow
    ...
    #  says what it is.
    dev_n = str(payload["rows"][0].get("n_scenes", 84))
    matcher_row = None
    if audit:
    ...
    for row in payload["rows"]:
        ...
        lines.append(
            f"{tex_escape(label)} & {dev_n} & {fmt(row['f1'])} & "
