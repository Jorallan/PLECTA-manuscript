...
\begin{tikzpicture}[pk, x=1mm, y=1mm]
  ...
  \node[note, anchor=west] at (87, -13)
    {eight rounds; frames are re-fitted\\
     along the assembled chains before\\
     each, and the last round is returned};

  % ---- what the method reads, stated once --------------------------------
  %  Anchored at the mask box's own left edge (x = 8), not centred on the
  %  box: centred, the note's left half stuck out past the drawing's left
  %  margin, widened the bounding box to 166 mm, and the whole picture
  %  overflowed \linewidth by 35 pt.
  \node[note, anchor=south west] at (8, 10)
    {procedural, or segmented from a micrograph
     (Section~\ref{sec:res-real})};

  % ---- the evaluated boundary -------------------------------------------
  \begin{scope}[on background layer]
    \node[scope, fit=(graph)(frame)(match)(bridge)] (core) {};
  \end{scope}
  \node[scopelab, anchor=north east] at (156.4, -35.5)
    {evaluated in this paper: one binary mask in, overlapping instances out};

  % ---- the depth stage ----------------------------------------------------
  %  Centred to the right of the output arrow's x so the two never cross.
  \node[note, anchor=center] at (94, -44)
    {reads a greyscale image of the same field as well as the mask; with no
     image every crossing abstains};
