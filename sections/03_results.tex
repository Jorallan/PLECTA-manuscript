...
Across the \PlectaHeldoutSceneCount{} held-out synthetic scenes, PLECTA
...
areal coverage. These scenes used previously unseen seeds from the development
generator; evaluation on an independent generator is reported in
...
Areal coverage describes the rendered scene, whereas PLECTA operates on its
centreline mask. We therefore examined whether the observed performance decline
reflects strand width or centreline length density.
Across \PlectaFactorialSceneCount{} development scenes, rendering each geometry
at three bundle widths left the input mask and overlap-aware reference
unchanged while areal coverage varied by up to a factor of
\PlectaFactorialMaxCoverageRatio{}. The corresponding variation in \Fone{} was
\PlectaFactorialMaxFOneRange{}. Increasing centreline length
density reduced mean \Fone{} from \PlectaFactorialLowFOne{} to
...
reconstruction from clean and degraded masks of the same scenes. On the
\PlectaRobustnessSceneCount{}-scene controlled set,
