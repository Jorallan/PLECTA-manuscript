...
We first evaluate strand segmentation on synthetic scenes
...
\subsection{Instance segmentation on synthetic images}
\label{sec:res-density}

These scenes carry exact reference identities, so every junction and gap
decision can be checked against the grouping the generator recorded when it
drew them.
...
Table~\ref{tab:plecta-heldout} summarises segmentation scores overall and by
areal coverage.

\begin{table}[tbp]
    \centering
    \caption{Segmentation scores overall and by target areal
...
PLECTA operates on the strand-axis mask, so segmentation difficulty depends
...
bundle-width distribution and may therefore produce different segmentation
...
segmentation from clean and degraded masks of the same scenes. Across the
...
irregularities therefore caused a small reduction in segmentation accuracy
...
Figure~\ref{fig:examples} shows representative segmentations at three
...
\begin{figure}[tbp]
    ...
    \caption{Segmentation at three coverage levels, each the scene closest
...
The stages above segment strands in projection but do not establish which
...
axis masks include segmentation error. Table~\ref{tab:depth-order} reports
...
\subsection{Instance segmentation on CNT SEM images}
\label{sec:res-real}
...
automatic axis evaluates the complete image-to-instances workflow.
Figure~\ref{fig:real-sem} shows both conditions on one whole field.

\begin{figure}[tbp]
    \centering
    \includegraphics[width=\linewidth]{figures/fig_real_sem.pdf}
    \caption{Segmentation of the whole B58\_110 field from two input-mask
    sources; the scale bar in (a) is 500\,nm. B58\_110 is the densest of the
    three fields and the lowest-scoring on the reference-derived axis. \textbf{(a)}~Reference
...
\begin{table}[tbp]
    \centering
    \caption{Segmentation from reference-derived and automatic axes in three
...
contribution of PLECTA's segmentation components, we removed one component
...
effects indicate that these components refine the core segmentation rather
than replace it.
...
The ablations identify the components that support segmentation. We next
...
All methods therefore received the same masks and were scored by the same
code. Configurations were selected on the same development scenes used for
PLECTA, separate from the scored ones, rather than left at their defaults. The
ranking also holds on GraFT's independently published generator, reported
below. The injection points, configurations and native end-to-end results are
documented in \suppcomparators{}.
...
\begin{figure}[!t]
  ...
  \caption{Five methods on identical masks, by coverage. The upper row uses
    clean axes and the lower row degraded axes. Columns: pairwise \Fone{},
    which counts pairs of fragments, and detection \Fone{} at IoU
    \PlectaDetectionIoU{} with a \PlectaPlacementTolerance{}\,px tolerance,
    which counts objects (Section~\ref{sec:mm-metrics}). The two disagree
    where a method fragments, since only the first is quadratic in
    fragmentation. Crossing Fidelity is reported in
    Table~\ref{tab:plecta-comparators}. $^{*}$\,our reimplementation; no
    implementation was available.}
...
alone is insufficient for the full segmentation task. Detailed results are
...
recreation of its sparse cobweb topology, PLECTA and SIFNE were similar
...
difference, although the recreated topology is weaker evidence than a
released generator.
...
show PLECTA's advantage for matched-mask segmentation in dense crossing
