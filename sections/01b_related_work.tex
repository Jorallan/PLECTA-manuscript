...
The problem of Section~\ref{sec:intro} touches several branches of prior
work: tools that measure strand images without resolving instances, methods
that recover instances by greedy continuation, methods that decide a whole
neighbourhood at once, complete micrograph-to-measurement workflows, and depth
ordering from a single view. This section takes them in turn.

\subsection{Tools that stop short of instances}
\label{sec:rel-tools}
...
than a mask carries. The stronger of Wegmayr et al.'s two variants drops the
angle rule and
...
\subsection{Deciding a whole neighbourhood at once}
\label{sec:rel-joint}
...
evaluators on fibre number, length and width \citep{peters2026fibre}. That
work and this one divide the task at the same joint and differ in what the
instance step is asked to prove: their tracer substitutes for human counting
and measurement, while PLECTA is isolated behind a binary mask and scored on
identity through crossings against overlap-aware references in which every
pairing is known.
