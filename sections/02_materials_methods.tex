...
unmatched incurs a penalty $p_i$, so linking stubs $i$ and $j$ gives a net
benefit of $p_i+p_j-C_{ij}$. Only links with a positive benefit are admitted.
Exact maximum-weight partial matching then selects the set of disjoint stub
pairs $\mathcal P$ with the greatest total benefit:
\begin{equation}
  \max_{\mathcal P}\sum_{(i,j)\in\mathcal P}
